...
part 'lesson_plan_history_state.dart';

class LessonPlanHistoryCubit extends Cubit<LessonPlanHistoryState> {
  LessonPlanHistoryCubit() : super(LessonPlanHistoryInitialState());

  List<LessonPlanHistoryModel> _allHistory = [];
  List<LessonPlanHistoryModel> _filteredHistory = [];

  /// Load all lesson plan history from Hive
  void loadHistory() async {
    try {
      emit(LessonPlanHistoryLoadingState());

      _allHistory = HiveService.getLessonPlansSorted();
      _filteredHistory = List.from(_allHistory);

      emit(LessonPlanHistoryLoadedState(history: _filteredHistory));
    } catch (e) {
      log('Error loading lesson plan history: $e');
      emit(LessonPlanHistoryErrorState(
  ...
  String? getChapter(LessonPlanHistoryModel item) {
    return _getChapterFromData(item.data);
  }

  /// Get creator from data (if available)
  String? getCreator(LessonPlanHistoryModel item) {
    try {
  ...
  void deleteLessonPlan(String lessonPlanID) async {
    try {
      await HiveService.deleteLessonPlan(lessonPlanID);
      loadHistory(); // Reload history after deletion
    } catch (e) {
      log('Error deleting lesson plan: $e');
      emit(LessonPlanHistoryErrorState(errorMessage: 'Failed to delete: $e'));
    }
  }
